...
- **Guardrails (Past Dates):** If the user asks to schedule an event or reminder for a date/time that has already passed, DO NOT register it. Instead, ask for clarification (e.g., "That date has passed, did you mean next year?"). The tools will block past-date registration and return an error message if you try.
