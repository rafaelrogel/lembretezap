...
### 3. Redis and Docker Compose

**Production:** Do not expose Redis port 6379 to the internet. Use `docker-compose.prod.yml` (or `docker-compose.vps.yml` generated by `install_vps.sh`), which overrides Redis to bind only internally. See DEPLOY.md §11.
...
### 4.1 Data Isolation Between Users (Multi-tenant)
