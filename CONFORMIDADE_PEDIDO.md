...
| **Livros/músicas/agenda** como comandos | `/filme`, `/livro`, `/musica`, `/agenda` no parser | Mapeados para ListTool; unificados no sistema de listas. |
...
- **Conforme:** Base zapista + WhatsApp, escopo organizador, filtro de scope (LLM + regex), comandos /lembrete, /list, /feito, /filme (e livros/musica/agenda como listas), memória per-user, PII truncada, delete pós-confirma sem histórico, audit log, rate-limit, FastAPI, Docker, cron nativo, estrutura DB e tools, prompts, testes, MVP local.
