...
_TIME_RESPONSE_PATTERNS = (
    ...
    r"(em|en|in)\s+\d+\s*(min|minuto|hora|hour|dia|day)",
    r"daqui\s+a\s+\d+",
    r"(?:todo\s+dia|diariamente|daily)\s+(?:às?|as|at)?\s*\d+",
...
def looks_like_time_response(content: str) -> bool:
    """True se parece uma resposta do utilizador com uma data/hora explícita."""
    if not content:
        return False
...
def last_assistant_asked_when(last_assistant: str) -> bool:
    ...
    # Matches the exact phrases generated by recurring_detector.py across ALL 4 languages
    return (
        # Combination of when/time + reminder
        (("quando" in t or "when" in t or "cuando" in t or "momento" in t or "horario" in t or "time" in t) and 
         ("lembrete" in t or "reminder" in t or "recordatorio" in t or "remember" in t or "lembrado" in t or "lembrada" in t or "recuerde" in t or "reminded" in t))
    ) or (
        # Phrase matches for 'agendar', 'horario', 'schedule', 'time'
        "agendar" in t or "schedule" in t or 
        "diz o horario" in t or "diz-me o horario" in t or "dime el horario" in t or
        "melhor horario" in t or "mejor horario" in t or "best time" in t or "tell me the time" in t
    ) or (
...
Regras:
- Prioriza a intenção MAIS RECENTE do utilizador. Ignora mensagens de dias atrás se houver uma actual sobre o mesmo tema.
...
Extrai da conversa:
1) O TEXTO do lembrete (procura na mensagem MAIS RECENTE do utilizador que contenha o assunto, ignorando discussões de dias atrás)
...
Regras:
- Ignora mensagens antigas (ex: "[4 days ago]") se houver uma mensagem recente ([Just now], [X min ago]) com um tema diferente.
- the message "texto" deve ser o assunto do lembrete (ex: "beber água", "reunião").
