...
def get_config_path() -> Path:
    ...
    return Path.home() / ".zapista" / "config.json"


def get_data_dir() -> Path:
    """Get the Zapista data directory."""
    from zapista.utils.helpers import get_data_path
...
def load_config(config_path: Path | None = None) -> Config:
    ...
    if path.exists():
        try:
            ...
            # Env override for Docker/deploy (e.g. ZAPISTA_CHANNELS__WHATSAPP__BRIDGE_URL=ws://bridge:3001)
            bridge_url = os.environ.get("ZAPISTA_CHANNELS__WHATSAPP__BRIDGE_URL")
...
def _apply_provider_env_overrides(data: dict) -> None:
    """Override provider API keys from env (ZAPISTA_PROVIDERS__DEEPSEEK__API_KEY, etc.)."""
    providers = data.get("providers")
    if not isinstance(providers, dict):
        return
    env_keys = (
        ("deepseek", "ZAPISTA_PROVIDERS__DEEPSEEK__API_KEY"),
        ("xiaomi", "ZAPISTA_PROVIDERS__XIAOMI__API_KEY"),
        ("openrouter", "ZAPISTA_PROVIDERS__OPENROUTER__API_KEY"),
        ("perplexity", "ZAPISTA_PROVIDERS__PERPLEXITY__API_KEY"),
    )
